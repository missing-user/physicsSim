class SpatialHash {
	...
	getHash(x, y) {
		return [~~(x / this.cellSize) * particles.cellSize, ~~(y / this.cellSize) * particles.cellSize].join(';')
	}
	add(obj) {
	...
	addRect(obj) {
		for (var i = this.closestCell(obj.x); i <= this.closestCell(obj.x + obj.w); i += this.cellSize)
			for (var j = this.closestCell(obj.y); j <= this.closestCell(obj.y + obj.h); j += this.cellSize) {
				let hash = [i, j].join(';')
	...
	addCirc(obj) {
		...
		maxDiag *= maxDiag
		for (var i = this.closestCell(obj.x - obj.r); i <= this.closestCell(obj.x + obj.r); i += this.cellSize)
			for (var j = this.closestCell(obj.y - obj.r); j <= this.closestCell(obj.y + obj.r); j += this.cellSize) {
	...
	getCirc(obj) {
		...
		for (var i = this.closestCell(obj.x - obj.r); i <= this.closestCell(obj.x + obj.r); i += this.cellSize)
			for (var j = this.closestCell(obj.y - obj.r); j <= this.closestCell(obj.y + obj.r); j += this.cellSize) {
				...
				if ((cx) * (cx) + (cy) * (cy) < maxDiag) {
					let hash = [i, j].join(';')
					if (hash in this.cells) returnBuffer = returnBuffer.concat(this.cells[hash])
				}
			}
