...
function loop() {
	...
	for (p of particles.objects) {
		...
		if (p.x - (p.r ?? 0) < 0) {
			p.vx = Math.abs(p.vx);
			p.x += 0.5; //moves the objects out of walls, even if their velocity is 0
		}
		if (p.y - (p.r ?? 0) < 0) {
			p.vy = Math.abs(p.vy);
			p.y += 0.5;
...
function hardCollision(a, b) {
	...
	if (a.x > b.x) if (b.vx < a.vx) return false;
	if (a.x < b.x) if (b.vx > a.vx) return false;
	if (a.y > b.y) if (b.vy < a.vy) return false;
	if (a.y < b.y) if (b.vy > a.vy) return false;
	f1 = (2 * a.m) / (a.m + b.m);
	f2 = (a.m - b.m) / (a.m + b.m);
...
function initialize() {
	...
	console.log("statics", statics.cells);
	loop();
}
initialize();
